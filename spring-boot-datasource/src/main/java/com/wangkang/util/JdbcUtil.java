...
public class JdbcUtil {
    ...
    //获得url
    private static String URL = "jdbc:mysql://localhost:3306/test?useUnicode=true&serverTimezone=GMT%2B8";
    //获得连接数据库的用户名
    ...
    public static void getTablesInfo(){
        ...
        try {
             ...
             */
            rs = dbmd.getTables(null, null, "qrtz_triggers", new String[]{"TABLE","VIEW"});
    ...
    public static void getPrimaryKeysInfo() {
        System.out.println("----------------- primary keys info ------------------");
        Connection conn =  getConnection();
        ResultSet rs = null;
        try{
            DatabaseMetaData dbmd = conn.getMetaData();
             ...
             */
            rs = dbmd.getPrimaryKeys("test","test","qrtz_triggers");
    ...
    public static void getIndexInfo() {
        ...
        try{
             ...
             */
            rs = dbmd.getIndexInfo("test", "test", "qrtz_triggers", false, true);
            while (rs.next()){
                String tableCat = rs.getString("TABLE_CAT");  //表类别(可为null)
                String tableSchemaName = rs.getString("TABLE_SCHEM");//表模式（可能为空）,在oracle中获取的是命名空间,其它数据库未知
                String tableName = rs.getString("TABLE_NAME");  //表名
    ...
    public static void getColumnsInfo(){
        ...
        try{
             ...
             */
            rs =dbmd.getColumns(null, "test", "qrtz_triggers", null);
